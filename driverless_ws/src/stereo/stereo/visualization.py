# import fsdv.perceptions.lidar.collect as collect
# import fsdv.perceptions.lidar.color as color
# import fsdv.perceptions.lidar.filter as filter
# import fsdv.perceptions.lidar.cluster as cluster
...
EXTRINSIC_HESAI = np.array([
		0.016235738570159631,
		-0.31985000396311664,
		0.94732907469256733,
		0.0,
		-0.9998581475216799,
		-0.00094687005696541138,
		0.016816309690593036,
		0.0,
		-0.0044816991862620366,
		-0.94746771892338888,
		-0.31982000558206852,
		0.0,
		0.10539401149491658,
...
def init_visualizer_window(name="Point Cloud Visualization"):
    ...
    vis.create_window(window_name=name, width=1500, height=750)
    return vis
...
def update_visualizer_window(vis, points, pred_cones=[], colors=None, colors_cones=[0, 1, 0], plane=None):
    ...
    if vis is not None:
        ...
        update_visualizer_perspective(vis, EXTRINSIC_HESAI)
...
def color_matrix(fns=None, pcs=None):
    '''
        Written By: Meher Mankikar (mm

# EXTRINSIC_HESAI = np.zeros((4,4))ankika) 10/22/2022
